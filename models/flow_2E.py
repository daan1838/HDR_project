...
class RAPID_Flow_Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = ptlflow.get_model('raft', 'sintel').eval().cuda()
    def forward(self, ldrs, expos):
        ...
        cur_adj_exp = cur_adj_exp.unsqueeze(1)
        nxt = nxt.unsqueeze(1)
        p_input = { "images" : torch.cat((prev,cur_adj_exp), dim=1) }
        n_input = { "images" : torch.cat((cur_adj_exp,nxt), dim=1) }

        p_flow = self.model(p_input)["flows"].squeeze(dim=1)
        n_flow = self.model(n_input)["flows"].squeeze(dim=1) 
        #p_flow 
